...
extern int       bufferEquity=0; // use this to emulate transfers between accounts. Start with 200, add 10,000. Only 200 will be seen by EA
...
bool     ma_close,profit_close[9999],trigger_reached[9999],order_exists[9999],res;
double   current_profit[9999],tkt_open[9999],tkt_high[9999],tkt_low[9999],tkt_close[9999];
int      hedge_tkt[9999],h_tkt;
double   symbol_profit;
double   f_profit[9999];
double   RSIprev;
int      open_trades[10000],open_tickets;
...
void CheckForClose()
  {
   ...
   for(int i=0;i<=OrdersTotal();i++)
     ...
     if(close_up && instant_close==true) reinit();
  } // end void
//+------------------------------------------------------------------+
...
// 1% increase then close
   ...
   */
   ...
   //CloseOutPrice=AccountEquity()+10;
   Print("MAGICMA - ",MAGICMA," CloseOutPrice - ",CloseOutPrice);
